...
(function ($) {
    ...
    /*=====================
        ...
        window.localStorage.setItem("isinchat","false");
        ...
        function newMessage() {
            ...
            if($("#direct").hasClass("active")){
                ident = '.messages .chatappend';
            }
            else{
                ident = '#groupchatlist';
            }

            $("<li class=\"replies\" id=\""+rand+"\"> <div class=\"d-flex\"> <div class=\"profile mr-4 bg-size\" style=\"background-image: url('"+$("#avatarimg").attr("src")+"'); background-size: cover; background-position: center center;\"></div><div class=\"flex-grow-1\"> <div class=\"contact-name\"> <h5>"+window.localStorage.getItem("myname")+"</h5> <h6>01:42 AM</h6> <ul class=\"msg-box\"> <li> <h5>" + message + "</h5> </li></ul> </div></div></div></li>").appendTo($(ident));
           ...
           // $(".messages").animate({ scrollTop:$(v).offset().top}, "fast");
        };
  ...
  $("#attachpics").click(function(){
    ...
    input.addEventListener("change", async function({target}){
        ...
        if (target.files && target.files.length) {
            ...
            window.localStorage.setItem("pics","");

            uploadpic(target.files,0);
...
function uploadpic(files,b){

    var extension = files[b].name.split('.').pop(); 
            
    window.localStorage.setItem("picpos",b);

    const uploadedImageBase64 = convertFileToBase64(files[b]); 
    ...
    var mountainsRef = storageRef.child(rand+'.'+extension);

    mountainsRef.put(files[b]).then((snapshot) => {
    ...
    .then((url) => {
       ...
       // addToDB("","images",url);
        ...
        let pics =     window.localStorage.getItem("pics");
        pics += url+",";
        window.localStorage.setItem("pics",pics);
        
        if(b>=files.length-1){
        rand = generateString(10);
        rand = rand.replace(" ","");

        pics =     window.localStorage.getItem("pics");
            
     
        let src = $("#picreply").html();
        src = src.replace("{theid}",rand);
        src = src.replace("{myname}",window.localStorage.getItem("myname"));
        src = src.replace("{avatar}",$("#avatarimg").attr("src"));
...
$("#attachfile").click(function(){
    ...
    input.addEventListener("change", async function({target}){
           ...
           if (target.files && target.files.length) {
            ...
            var extension = target.files[0].name.split('.').pop(); 
            ...
            .then((url) => {
                ...
                addToDB("","attachment",url+"<>"+target.files[0].name);

                rand = generateString(10);
                rand = rand.replace(" ","");
                let src = $("#get").html();
                src = src.replace("{id}",rand);
                src = src.replace("{profileurl}",$("#avatarimg").attr("src"));
                src = src.replace("{fileurl}",url);
                src = src.replace("{filename}",target.files[0].name);
    ...
    input.click();
        ...
        function addToDB(message,kind,files){
            ...
            citiesRef.where("uname", "==", window.localStorage.getItem("user")).get().then((querySnapshot) => {
                ...
                querySnapshot.forEach((doc) => {
                        ...
                        arr.push({ from:window.localStorage.getItem("user"), kind:kind, files:files, themessage:message });
                ...
                if(window.localStorage.getItem("isinchat")=="false"){

                    citiesRef.where("uname", "==",  window.localStorage.getItem("user")).get().then((querySnapshot) => {
    
                        querySnapshot.forEach((doc) => {
                      
                          doc.ref.collection('chats').doc(window.localStorage.getItem("talkingto")).set({
                           from: window.localStorage.getItem("talkingto"),
                           logs: []
                        })
                        .then(() => {
           ...
           //ADD TO INCOMING MESSAGE OF TALKING TO
             
            citiesRef.where("uname", "==", window.localStorage.getItem("talkingto")).get().then((querySnapshot) => {

                querySnapshot.forEach((doc) => {
             
                let arr = doc.data().incoming_actions;
            ...
            var docRef = firestore.collection("group_chats").get().then((querySnapshot) => {
                querySnapshot.forEach((doc) => {
                  ...
                  if(doc.id==window.localStorage.getItem("selectedGroup")){
                    ...
                    sendtoMember(doc.data().members,0,kind,files,message);
        ...
        function sendtoMember(members, i,kind,files,message){

        
       
            if(members[i]!=window.localStorage.getItem("user")){
               
            const callDoc = firestore.collection('users')
            var citiesRef = callDoc.doc("users").collection('list');
            
            citiesRef.where("uname", "==",members[i] ).get().then((querySnapshot) => {
                ...
                querySnapshot.forEach((doc) => {
                    ...
                    arr.push({ 
                        ...
                        action:"incoming_message",
                        kind: kind,
                        group: window.localStorage.getItem("selectedGroup"),
                        files: files,
                        message: message
                     });
                    ...
                    i+= 1;
                    sendtoMember(members,i,kind,files,message);
                    }
    ...
    FOCUS ON THIS STICKER SENDING
       ...
       $('.sticker-contain ul li').on('click', function (e) {
        ...
        $("<li class=\"replies\" id=\""+rand+"\"> <div class=\"d-flex\"> <div class=\"profile mr-4 bg-size\" style=\"background-image: url('"+$("#avatarimg").attr("src")+"'); background-size: cover; background-position: center center;\"></div><div class=\"flex-grow-1\"> <div class=\"contact-name\"> <h5>"+window.localStorage.getItem("myname")+"</h5> <h6>01:42 AM</h6> <ul class=\"msg-box\"> <li> <h5>" + sticker + "</h5> </li></ul> </div></div></div></li>").appendTo($(ident));
